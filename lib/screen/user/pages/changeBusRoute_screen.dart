          ...
          children: [
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Obx(() => 
                            Column(
                              children: userBusList.asMap().entries.map((entry) {
                                int index = entry.key;
                                UserBus busCity = entry.value;
                                return BusLoadCheckButtonWidget(
                                  busCity: busCity,
                                  dropDownKey: _dropDownKey,
                                  isSelected: _selectedBusIndex.value == index, // 버튼이 선택되었는지 여부
                                  onTap: () {
                                    if (index >= 0 && index < userBusList.length) {
                                      _selectedBusIndex.value = index; // 유효한 인덱스인지 확인
                                      _dropDownKey.currentState?.updateDropdownItems(busCity); // 드롭다운 항목 업데이트
                                    }
                                  },
                                ).pOnly(bottom: 10);
                              }).toList(),
            ...
            SubmitButton(
              onPressed: () async {
                ...
                await busCityController.sendUserTakeBusCheck(isTake: isTakingBus);
              },
              title: '신청하기'
            ).pOnly(bottom: 25)
          ],
